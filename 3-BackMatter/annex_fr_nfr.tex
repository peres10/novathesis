\typeout{NT FILE annex_fr_nfr.tex}%

\chapter{Annex 1 - Functional and Non-Functional Requirements}
\label{ann:fr_nfr_annex}
...

{
    \begin{table}[H]
        \centering
        \caption{Functional Requirements - Student Candidacies and Seriation}
        \begin{tabular}{|l|p{12cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            FR-31 & The system shall allow Students to view all available propsals for their enrolled edition and rank a specified numbre of preferred proposals in preference order. \\ \hline
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{12cm}|} % 'l' for ID, 'p' for wrapped text
            ...
            FR-42 & The system shall Company Representatives to manage their organizational hierarchy, icluding parent company designation and sub-entities/departaments, and to update company contact  details and registration information \\ \hline
            ...
            FR-44 & The system shall provide “Superuser” functionality, allowing Edition Coordinators to temporarly access other user accounts using only the their user’s identifier (student number or username) for support and troubleshooting purposes, with full audit logging of such acccess. \\ \hline
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{12cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            FR-51 & The system shall automate the “Call for Proposals” process, sending notificatons to all registered companies (that have an active status) at the start of each edition, specifying submission deadlines and edition type. \\ \hline
            FR-52 & The system shall allow the publication of news and annoucements targeted at specific user roles (e.g., news visible only to Students) \\ \hline
            ...
            FR-54 & The system shall support both email and in-app notifications for critical events (new proposals, feedback requests, deadline remiders, seriation results) \\ \hline
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{12cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            FR-61 & The system shall allow Edition Coordinators and Adminitrators to export lists of students, proposals, candidacies, and final assignemnts in open standard formats (CSV, JSON). \\ \hline
            ...
            FR-63 & The system shall provide audiatable records of all proposal feedback exchanges (coordinator feedback, company responses, timestamp of modications) for compliance and dispute resolution. \\ \hline
        \end{tabular}
    \end{table}    
}

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{12cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            FR-71 & The system shall support specific workflows for Master’s degrees, including the distinction between scientific dissertations and engineering proejcts. \\ \hline
            ...
            FR-74 & The system should have edition for the Master’s degree scientific dissertation and engineering projects that work the same way as the undergraduate internships, with just some chages. \\ \hline
        \end{tabular}
    \end{table}    
}
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            ...
            NFR-SEC-2 & All data in transit shall be encrypted uting HTTPS/TLS, preventing interception of sensitive information during transmission. \\ \hline
            NFR-SEC-3 & Password and sensitive credentials shall be hashed using industry-standard alogirthms (bcrypt, PBKDF2, or Argon2) and shall never be stored in plaintext. \\ \hline
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            NFR-REL-1 & The transaction email service shall guarantee a good deliverability rate to ensure time-sensitive notifications (eg,. proposals deadlines, seriation results) reach intented recipients reliably \\ \hline
            ...
            NFR-REL-3 & The system shall implement health monitoring and automated alerts for critical failures (database unavailability, backup failures, email service degradation), enabling rapid response by adminstrative staff. \\ \hline
        \end{tabular}
    \end{table}    
}

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            NFR-PERF-1 & All user-facing pages (dashboards, proposal lists, candidacy tracking) shall load within 3 seconds under normal load conditions. \\ \hline
            ...
            NFR-PERF-3 & API endpoints shall respond withing 500 ms for 95\% of requests under typical load, and within 2 secods for 99\% of requests. \\ \hline
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            ...
            NFR-USAB-3 & New user workflows (registration, proposal submission, candidacy ranking) shall be completable without raining or external documentation, with clear inline guidance and error messages. \\ \hline
        \end{tabular}
    \end{table}    
}
...

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            ...
            NFR-MAINT-4 & Configuration (database URLs, email credentials, feature flags) shall be externalized using environemnt variables or configuration files, not hardcoded, to enable deployment across development, staging and production environments. \\ \hline
        \end{tabular}
    \end{table}    
}

{
    \begin{table}[H]
        ...
        \begin{tabular}{|l|p{11cm}|} % 'l' for ID, 'p' for wrapped text
            \hline
            \textbf{ID} & \textbf{Description} \\ \hline
            NFR-EMAIL-1 & All outbount emails shall implement sender authentication protocols to minimize spam flagging \\ \hline
