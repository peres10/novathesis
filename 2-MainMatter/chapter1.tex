...
\section{Context and Background}
  ...
  \ntindex[Template]{}
  {
    ...
    market or advanced research is a key part of the university experience. 
    This is mainly supported through three different distinct paths at the 
    \gls{DI} of \gls{FCT}:
  }
  ...
  \begin{itemize}
    \item {\textbf{Undergraduate Internships}: Primarily offered by external companies to provide students with their first professional experience.}
    \item {\textbf{Master's Dissertations}: Scientific thesis typically conducted within the university, focusing on academic research under the supervision of professors.}
    \item {\textbf{Engineering Projects}: Practical projects proposed by external companies or the \gls{DI} itself, allowing Master's students to apply advanced engineering principles to real-world projects.}
  \end{itemize}

  Managing those processes involves a complex coordination of multiple stakeholders: 
  students seeking opportunities aligned with their skills and interests, 
  professors coordinating editions, 
  proposing dissertation themes and supervising students, and external companies providing internships and engineering projects. 
...
\section{Problem Statement}
  ...
  \ntindex[Template]{}
  At \glsxtrshort{DI} at \glsxtrshort{NOVA} \glsxtrshort{FCT}, the management of internships, dissertations and engineering projects 
  involves a multi-stage lifecycle that includes proposal submission, clarification and approval processes, 
  making proposals available to students, assigning projects to students, report/dissertation submission, and providing documentation to juries.
  Currently, the department relies on a legacy platform to support some of these activities, however this system 
  presents several critical issues that hinder the efficiency of the department.
  \\
  ...
  or non-standardized methods. This creates a lack of centralized data and increases the administrative 
  burden on coordinators, professors and the secretariat.
  \\
  Furthermore, even within its current scope, the legacy system suffers from several functional and technological gaps:

  \begin{itemize}
    \item {\textbf{Communication Gaps and Reliability}: The current method for “\textit{Call for Proposals}” is manual, 
      requiring coordinators to send individual emails to companies, advertising the opportunity to propose projects, 
      ...
      editions or context.}
    \item {\textbf{Inefficient Feedback Loops}: Although companies can submit proposals, there is no integrated mechanism 
      ...
      to correct and resubmit proposals based on coordinator comments.}
    \item {\textbf{Stale Company Data}: The existing repository contains a large amount of outdated information, 
    ...
    \item {\textbf{Manual Protocol Management}: For new companies that have never participated in previous editions, 
      the process of signing the required protocols is still entirely manual. The platform should automate 
      delivery and tracking of these legal documents as soon as a new company registers.}
    \item {
      \textbf{Outdated Technology and \gls{UX}}: The platform is built on an obsolete technology stack that is hard
      to maintain, difficult to update, and exposes security risks. 
      At the same time, the user interface is not intuitive for external stakeholders, making it 
      difficult for companies to register, manage employee accounts, or monitor the status 
      of their proposals in real time.
    }
  \end{itemize}

  {  
    In conclusion, the necessity for a new platform arises from the need to unify all academic degrees under a 
    single digital infrastructure, replacing manual tracking with a secure, automated system that ensures data 
...
\section{Objectives}
  ...
  and engineering projects for \glsxtrshort{DI} at \glsxtrshort{NOVA} \glsxtrshort{FCT}. The projects aim to transition from a fragmented, manual system 
  ...
  \begin{itemize}
    \item {\textbf{Process Unification}: Integrate the lifecycles of undergraduate internships and Master's degree pathways (both scientific and engineering-focused) into a single platform, replacing the current manual methods.}
    \item {\textbf{Workflow Automation}: Implement automated procedures for the "\textit{Call for Proposals}", the validation of proposals, the student \textit{seriation} (ranking) and the assignment of students to a proposal.}
    \item {\textbf{Enhanced Communication and Feedback}: Establish direct feedback loops within the platform, allowing coordinators to request clarifications on proposals and ensuring companies can resubmit corrected versions.}
    \item {\textbf{Document and Protocol Management}: Automate the delivery, tracking, and storage of legal protocols, student CVs, and recommendation letters.}
    \item {\textbf{Administrative Oversight}: Develop advanced administrative tools, including “\textit{superuser}” access for student support, detailed system logs for auditing, and validation workflow for new company registrations.}
  \end{itemize}
...
\section{Methodology}
  ...
  \ntindex[Template]{}
  This project will follow an Agile/Scrum methodology, 
  characterized by iterative development, 
  frequent feedback loops \cite{scrum-guide-2020} to ensure the final 
  platform aligns with the university’s complex requirements. 
  The work plan is structured into several phases, 
  prioritizing rapid prototyping followed by a transition to a 
  high-performance custom architecture.
  \\
  To achieve this goal, the following methodological steps 
  will be used for the design, implementation, and validation of the system:
  
  \begin{enumerate}
    \item{
      \textbf{Requirement Gathering and Analysis}: 
      A thorough analysis of the legacy system and current manual processes was conducted to identify functional and non-functional requirements. This phase includes the definition of user roles 
      (\textit{Admin}, \textit{Coordinator}, \textit{Student}, \textit{Professor}, \textit{Company}, \textit{Secretariat}) and their respective use cases.
      }
    \item{\textbf{System Modeling and Design}: 
      Utilizing \gls{BPMN} to map complex workflows \cite{omg-bpmn-2011} and \gls{ERD} 
      to design the database schema \cite{Chen1976ERModel}. This phase also involves 
      planning the system architecture.
      }
    \item{
      \textbf{\glsxtrshort{AI}-Assisted Rapid Prototyping (\textit{Vibe Coding})}: 
      The initial implementation phase will leverage \textit{Vibe Coding} tools to quickly 
      generate the frontend and core functional 
      interfaces. 
      In particular, the project will likely use Bolt.new as the main environment for prompt-to-full-stack prototyping, with 
      Lovable as an alternative option if needed, and Figma Make to support the design and generation of visually complex UI components \cite{vibecoding2025}. 
      These tools excel at creating user interfaces 
      through iterative natural language prompting. 
      During this stage, Supabase will be utilized as \gls{BaaS} to provide an immediate 
      infrastructure for authentication, database management, 
      and initial data structures via \glsxtrshort{SQL}\cite{supabase-2019}. 
      The prototype produced in this phase will serve as the baseline that will be iteratively improved and hardened.
      This approach allows for the rapid fulfillment of most functional requirements while 
      maintaining a live, testable version of the platform.
      }
    \item{
      \textbf{Progressive Refinement and Continuous Development}: 
      Once the prototype reaches a stable state and meets the core functional requirements,
      the focus shifts from "\textit{getting this working}" to improving code quality, structure, and user experience.
      In this phase the existing codebase generated by AI is refactored, extended, and optimized manually and potentially still
      with the help of \glsxtrshort{AI}-assisted tools. The goal is to evolve the prototype into a maintainable 
      system, improving \glsxtrshort{UX}, performance, and adherence to architectural guidelines.
      }
    \item{
      \textbf{Custom Backend Evolution}: 
      To support more complex business logic - such as specialized student \textit{seriation} (ranking) algorithms and robust administrative auditing - 
      the system progressively transitions towards a dedicated custom backend, 
      likely using Spring Boot \cite{springboot-docs-3x}. 
      While Supabase remains valuable for rapid development, a Spring Boot-based architecture provides stronger domain modelling, 
      type safety and maintainability through its layered structure 
      (Controller, Service, Repository). 
      This evolution builds on top of the existing data model and functionality, ensuring that the platform remains usable while 
      gaining scalability and reliability for long-term university use.
      }
    \item{\textbf{Verification and Validation}: 
      The platform will be tested against the initial requirements, 
      ensuring that automated emails follow specific deliverability rules avoiding being flagged as spam.
      Additionally, the seriation logic will be rigorously validated to ensure it correctly handles student placements and matching according to pre-defined criteria.
      }
    \item{
      \textbf{Documentation and Evaluation}: 
      There will be a continuous documentation of the architecture and design decisions, 
      providing detailed justifications for technical choices such as the preference for \glsxtrshort{SQL} 
      over \glsxtrshort{NoSQL} for structured academic data, and other technical choices.
      At the end, the platform and development process will be tested and evaluated with respect to the initial goals,
      highlighting limitations and possible directions for future work. 
      }
  \end{enumerate}

\section{Document Structure}
  \label{sec:intro_doc_structure}
  
  {
    This dissertation plan is organized into four main chapters, followed by appendices.
    Chapter~\ref{cha:introduction} introduces the context, problem statement, objectives, and methodology 
    adopted for the project.
  }

  {
    Chapter~\ref{cha:state_of_the_art} presents the state of the art and technological choices considered for the platform,
    covering backend architectures, data persistence models, frontend frameworks, and \glsxtrshort{AI}-assisted development tools,
    and concludes with a justification of the selected stack.
  }

  {
    Chapter~\ref{cha:requirements_and_system_analysis} analyzes the current legacy system and describes the elicitation of
    requirements for the new platform, including stakeholders, functional and non-functional requirements, and the main use
    cases that structure the system's behavior.
  }

  {
    Chapter~\ref{cha:solution_proposal} proposes the solution, detailing the overall system architecture, the \glsxtrshort{BPMN} models for
    core processes, the data model \glsxtrshort{ERD}, and the work plan that guides implementation and evaluation.
    The appendices provide the full catalog of requirements, detailed use case specifications, \glsxtrshort{BPMN} diagrams, and 
    the complete \glsxtrshort{ERD}.
  }
